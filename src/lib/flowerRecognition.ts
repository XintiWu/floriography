...
export async function recognizeFlowerFromBuffer(
  ...
  if (!apiKey) {
    throw new Error("GeminiApiKeyMissing");
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      throw new Error("GeminiInvalidResponse");
    } else {
      ...
      throw new Error("GeminiApiError");
    }
  } catch (err) {
